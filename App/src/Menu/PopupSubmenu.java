...
public class PopupSubmenu extends JPanel {
    ...
    public PopupSubmenu(ComponentOrientation orientation, Menu menu, int menuIndex, String menus[]) {
        this.menu = menu; // Đối tượng menu chính
        this.menuIndex = menuIndex; // Chỉ số menu cha
        this.menus = menus; // Danh sách menu
        applyComponentOrientation(orientation); // Hướng của menu (trái phải hoặc phải trái)
        init();
    }

    private void init() {
        setLayout(new MenuLayout());
        popup = new JPopupMenu(); // Chứa Submenu
        ...
        // Thêm các nút submenu
        for (int i = 1; i < menus.length; i++) {
            JButton button = createButtonItem(menus[i]); // Tạo JButton cho mỗi Submenu
            final int subIndex = i;
            // Thêm event khi nhấn vào menu
    ...
    // Căn chỉnh hướng của Menu chính    
